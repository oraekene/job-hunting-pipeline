...
7. Grep the docx text for **this application's** forbidden claims — derive the grep pattern from the genuine gaps in `keyword_analysis.json`/`resume_match.md` (`found_in_resume:false`), NOT from a fixed string. For example, a DevSecOps posting greps `DevSecOps|infrastructure|self-hosted|Principal|Kubernetes|Docker`; a robotics/hardware posting (like this one) greps `robotics|hardware|manufacturing|physical product|supply chain|manufacturability`. Expect zero hits under strict mode. **Use word-boundary regexes, never substring `in` checks**: `\bdefi\b` matches only "DeFi", but a naive `'defi' in text` also fires on "defin**ed**" (real false positive this session). The generator script's own grep (`re.search(pat, text, re.I)`) is the authoritative check; an independent re-check with the same word-boundary patterns is a good second opinion. **`--forbidden` patterns must stay FLAT** (no alternation groups `(a|b)`, no parens — the verifier splits the string on `|`; a group segment crashes with `re.error`). Keep complex alternations in the generator's embedded Python-list grep instead. **Read the verifier's per-line PASS/FAIL output, not just its final summary** — its bookkeeping can print FAILs yet still exit 0 (see `references/verifier-fallback-adhoc.md`).
...
- See `scripts/verify_app_artifacts.py` for the packaged verifier (JSON math recompute, resume_match.md header cross-check, generator run, docx forbidden grep, stale-reference grep): `python scripts/verify_app_artifacts.py shared/build_artifacts/app_N/ --forbidden "pat1|pat2" --stale "63|47"`. **⚠ FIXED 2026-08-17: `check()` previously required 3 args while every call site passed 2 → crashed on its first check for every app dir. The fix is `def check(cond, msg, fails=None)` with the append guarded by `fails is not None` — the script now exits 0 with ALL CHECKS PASSED (verified on app_20/21/22).** The sibling copy at `job-hunting-artifact-qa/scripts/verify_app_artifacts.py` still carries the old 3-arg bug — apply the same one-line fix there before relying on it. See `references/verifier-fallback-adhoc.md` for the tempfile `hermes-verify-*` replica pattern if a verifier copy is still broken.
...
- See `references/sweep-run-lessons.md` for session-verified operational lessons from the 2026-08-17 sweep (apps 20/21/22): live JD drift vs stale DB rows (score the live posting, app_21), dead stored URL ≠ gone posting (search the company careers site first, app_22), resuming a subagent build that died mid-way (preserve the surviving keyword_analysis.json, complete in place), the `hermes send` Telegram ping mechanics (delivered = `message_id` + `mirrored`, timeout → retry once, MSYS-vs-native Windows paths), and the separate `status='awaiting_approval'` transition after `--mark-approval-pinged`.
